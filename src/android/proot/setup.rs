...
fn setup_qterminal_wrapper(_: &SetupOptions) -> StageOutput {
    ...
    // KDE Plasma (the default DE) ships with Konsole rather than QTerminal, so on
    // a default install QTerminal isn't present and the wrapper would shadow
    // `qterminal` in $PATH with a script that immediately fails. Only install the
    // wrapper when QTerminal is actually present (e.g. user added it back via a
    // custom DE config).
    let qterminal_bin = fs_root.join("usr/bin/qterminal");
    if !qterminal_bin.exists() {
        return None;
    }
...
fn setup_plasma_scaling(options: &SetupOptions) -> StageOutput {
    let fs_root = Path::new(ARCH_FS_ROOT);
    ...
    apply_plasma_scaling(fs_root, density_dpi);
...
pub fn setup(android_app: AndroidApp) -> PolarBearBackend {
    ...
    let stages: Vec<SetupStage> = vec![
        ...
        Box::new(setup_qterminal_wrapper), // Step 5. Ensure qterminal launches interactive bash (no-op when QTerminal isn't installed; KDE Plasma uses Konsole)
        ...
        Box::new(setup_plasma_scaling),     // Step 8. Setup KDE Plasma HiDPI/font scaling, disable Baloo + DrKonqi
